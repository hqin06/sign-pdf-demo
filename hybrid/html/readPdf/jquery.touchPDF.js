	...
	function TouchPDF(element, options) {
		...
		function initDom() {
			...
			$element.addClass("touchPDF").html(
				'<div class="pdf-toolbar"></div><div class="pdf-outerdiv" id="my-pdf-outerdiv">'
				/*+ '<div class="pdf-tabs"></div>'*/
				+
				'<div class="pdf-viewer">' +
				'<div class="pdf-loading" style="font-size:38px;">' + options.loadingHTML + '</div>' +
				'<div class="pdf-drag">' +
			...
			if (options.showToolbar) {
				...
				$element.find(".pdf-toolbar").html(
					...
					'<div class="pdf-button"><button class="btn-css pdf-prev">上一页</button></div>' +
					'<div class="pdf-button"><span class="pdf-page-count"></span></div>' +
					'<div class="pdf-button"><button class="btn-css pdf-next">下一页</button></div>'
					// + (options.disableZoom? '':'<div class="pdf-button"><button class="btn-css pdf-zoomin">放大</button></div>'
					// 	+ '<div class="pdf-button"><button class="btn-css pdf-zoomout">缩小</button></div>')
				);
		...
		function renderPage() {
			...
			pdfDoc.getPage(pageNumRendering).then(function(page) {
				...
				$(".pdf-canvas").css("transform", "scale(" + (1 / options.quality) + ")").css(
		...
		function redraw() {
			...
			scale = Math.min(winHeight / (pdfHeight + TOOLBAR_HEIGHT + BORDER_WIDTH * 2), winWidth / (pdfWidth +
				tabWidth * 2 + BORDER_WIDTH * 2));
			if (scale > 1) scale = 1;
		...
		function renderAnnotations(page, viewport) {
			...
			page.getAnnotations().then(function(annotationsData) {
				...
				$.each(annotationsData, function(i, data) {
					if (!data || !data.hasHtml || data.subtype !== 'Link' || (!data.dest && !data
							.url)) return;
							...
							.on('click', function(e) {
								...
								if (dest instanceof Array) {
									getPageIndex(dest[0]).then(function(num) {
										if (state != LOADED && state != ZOOMEDIN)
											return;
										goto(num);
									});
								} else {
									pdfDoc.getDestination($(this).data("dest")).then(function(
										destRefs) {
										if (!(destRefs instanceof Array))
